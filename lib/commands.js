module.exports = {
  ...
  inactiveButton: async function(page, selector) {
    try {
        await page.waitForSelector(selector);
        // Возвращаем результат проверки наличия класса
        return await page.$eval(selector, button => {
            return button.classList.contains('movie-seances__time-block');
        });
    } catch (error) {
        throw new Error(`Not possible to check inactive button for selector: ${selector}`);
    }
  }
};
